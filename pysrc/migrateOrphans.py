...
Sweep model (dispatch on the legacy fileMap's per-entry `migrate:` disposition):
  ...
  - `port`    user code a later porter phase converts in place (`block`,
              `rtlModule`, `tbExternal`, `testBench`); NEVER expanded for
              deletion, so unreachable here. The tb top is `port` for its
              GENERATED_CODE_PARAM tail alone (it has no code slots): the porter
              carries the user's DUT `--variant=` onto the scaffolded `.cppm`
              before deleting the legacy pair.
...
#             the value is stamped onto exists.
...
def sweepOrphans(prj, write=False):
    ...
    the later porter phase, which converts it or reports a TODO for the operator to
    finish by hand. A same-form (non-parameterized)
