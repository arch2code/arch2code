...
class projectOpen:
    ...
    def resolveFileOwner(self, params):
        ...
        if params.parent:
            ...
            # param spelling is relative to the build root that generated the file
            # (a child project generated standalone differs from this build), so
            # canonicalize to the yamlContext key (shared with contextOwningProject)
            # before the owner lookup.
    ...
    def resolveContextKey(self, name):
        ...
        # line to its yamlContext key. An exact key is returned unchanged.
        # Otherwise the name is matched by basename against the known context
        # keys. yamlContext, includeName and contextOwningProject all share these
        # keys, so the resolved key indexes any of them. Exits on an ambiguous or
        # unknown name.
        #
        # A context name is written into a generated file relative to the build
        # root that produced that file. A file generated by a referenced child
        # project's own standalone build therefore carries a directory prefix
        # relative to the child's root, which differs from this build's key for
        # the same physical context file. Matching on the basename (stable across
        # build roots) lets the ownership gate still identify such a file's owner
        # and skip it, keeping single-project builds unaffected because their
        # names always hit the exact-key match above.
        if name in self.yamlContext:
            return name
        lookupBase = os.path.basename(name)
        matching_keys = []
        for key in self.yamlContext:
            base = os.path.basename(key)
            base_no_ext, _ = os.path.splitext(base)
            if lookupBase == base or lookupBase == base_no_ext:
