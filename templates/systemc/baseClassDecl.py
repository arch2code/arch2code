...
def render(args, prj, data):
    ...
    # Stage 3.2 of plan-variant-config-unification.md: Base/Inverted/Channels
    # companions inherit the parent's "leaf parameterizable" status. A block
    # is a class template only when it declares its own `params:`.
    ...
    # Active force-link function declaration for non-templated blocks.
    # Generated parent / testbench code calls force_link_<block>()
    # immediately before factory lookups that may construct this block.
    # The call creates a real symbol reference into <block>.cpp,
    # forcing the linker to pull that object into the program even when
    # nothing else references symbols from it. This is required under
    # C++20 modules and static-archive linking, where 'import' alone
    # does not guarantee that an imported module's implementation unit
    # is linked. Without this call, the self-registering static inside
    # <block>.cpp may not fire and the block may be unreachable through
    # the factory.
    #
    # Generated by templates/systemc/baseClassDecl.py (and
    # templates/systemc/constructor.py for the matching definition).
    # See plan-block-registration.md "Force-Link Function" for the full
    # rationale.
    #
    # Stage 3.2 of plan-variant-config-unification.md broadens the predicate
    # from "non-parameterizable" to "non-templated" (no own `params:`) so
    # that parent containers that are flagged isParameterizable solely
    # because parameterizable structures transit their surface (e.g.,
    # `ip_top`) also receive the force-link anchor.
    if not hasOwnParams:
        out.append('')
        out.append(f'// Force-link function (active modules-mode anchor). See plan-block-registration.md.')
        out.append(f'void force_link_{ data["blockName"] }();')
...
def renderClass(args, prj, data, blockName, ifMapping, isParameterizable=False):
    ...
    if ifMapping['addConsts']:
        if prj.data['blocks'][data['qualBlock']]['params']:
            # Stage 4.2 of plan-variant-config-unification.md: the
            # constants that mirror block params now read from the
            # Config policy directly. The class is templated on Config
            # for any block with own params (Stage 1.2), so the
            # qualified-id is well-formed at the constructor's
            # mem-initialiser. The runtime instanceFactory::getParam
            # path is decommissioned in Stage 4.4.
            for param in prj.data['blocks'][data['qualBlock']]['params']:
