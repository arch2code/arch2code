...
5.  **Connections (`connections` list):**
    ...
    *   **Hand-written RTL** in a domain whose clock is not literally named `clk` must use the explicit macro family (`` `DFF_CLK(clkSlow, q, d) ``, `` `DFF_INST_CLK(clkSlow, type, name) ``, and so on). The bare `` `DFF `` family expands to `clk`. See `rtl-core.md`.
