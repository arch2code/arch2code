...
template <typename D, typename U = std::monostate>
struct axiReadRespSt
{
    ...
    inline bool operator == (const axiReadRespSt< D, U > & rhs) const {
    ...
    inline friend void sc_trace(sc_trace_file *tf, const axiReadRespSt< D, U > & v, const std::string & NAME ) {
    ...
    inline friend ostream& operator << ( ostream& os,  axiReadRespSt< D, U > const & v ) {
        os << "("
...
struct axiReadAddressSt
{
    ...
    inline bool operator == (const axiReadAddressSt< A, U > & rhs) const {
        if constexpr (hasOptionalPayload<U>) {
            if (!(rhs.user == user)) { return false; }
        }
        return (
    ...
    inline friend void sc_trace(sc_trace_file *tf, const axiReadAddressSt< A, U > & v, const std::string & NAME ) {
    ...
    inline friend ostream& operator << ( ostream& os, axiReadAddressSt< A, U > const & v ) {
        os << "("
           << (int)v.arid << ", "
           << v.araddr.prt() << ", "
           << (int)v.arlen << ", "
...
template <class A, class D, class ARU = std::monostate, class RU = std::monostate>
...
public:
    ...
    // blocking read
    virtual void receiveAddr( axiReadAddressSt<A, ARU>& ) = 0;
    virtual void sendData( const axiReadRespSt<D, RU>& ) = 0;
...
private:
    // disabled
    axi_read_in_if( const axi_read_in_if<A, D, ARU, RU>& );
    axi_read_in_if<A, D, ARU, RU>& operator = ( const axi_read_in_if<A, D, ARU, RU>& );
};
...
public:
    // blocking write
    virtual void sendAddr( const axiReadAddressSt<A, ARU>&, std::optional<std::string> str=std::nullopt ) = 0;
    virtual void receiveData( axiReadRespSt<D, RU>& ) = 0;
    virtual void receiveDataCycle(  axiReadRespSt<D, RU>& ) = 0;  // for multi cycle channels, gets one burst
...
private:
    // disabled
    axi_read_out_if( const axi_read_out_if<A, D, ARU, RU>& );
    axi_read_out_if<A, D, ARU, RU>& operator = ( const axi_read_out_if<A, D, ARU, RU>& );
};
...
: public axi_read_in_if<A, D, ARU, RU>,
  public axi_read_out_if<A, D, ARU, RU>,
...
public:
    ...
    explicit axi_read_channel( const char* name_, std::string block_, std::string multiCycleType_, int pingpong_size_, std::string trackerName_, INTERFACE_AUTO_MODE autoMode=INTERFACE_AUTO_OFF)
      : sc_prim_channel( name_ ),
        m_addr_channel((std::string(name_) + "addr").c_str(), block_),
        m_data_channel((std::string(name_) + "data").c_str(), block_, multiCycleType_, pingpong_size_ * sizeof(axiReadRespSt<D, RU>), trackerName_),
        m_addr_out((std::string(name_) + "addrOut").c_str()),
        m_data_out((std::string(name_) + "dataOut").c_str()),
        m_addr_in((std::string(name_) + "addrIn").c_str()),
        m_data_in((std::string(name_) + "dataIn").c_str()),
    ...
    // out interfaces
    virtual void sendAddr( const axiReadAddressSt<A, ARU>&, std::optional<std::string> str ) override;
    virtual void receiveData( axiReadRespSt<D, RU>& ) override;
    virtual void receiveDataCycle( axiReadRespSt<D, RU>& ) override;
    virtual uint8_t * getReadPtr( void ) override;
    virtual void push_burst(uint32_t burstCount) override { m_data_in->push_context(burstCount * sizeof(axiReadRespSt<D, RU>)); }

    // in interfaces
    virtual void receiveAddr( axiReadAddressSt<A, ARU>& ) override;
    virtual void sendData( const axiReadRespSt<D, RU>& ) override;
...
protected:
    rdy_vld_channel<axiReadAddressSt<A, ARU>> m_addr_channel;
    rdy_vld_channel<axiReadRespSt<D, RU>> m_data_channel;
public: // make these public for the tees/verif
    rdy_vld_out <axiReadAddressSt<A, ARU>> m_addr_out; // for sending address (axi_out)
    rdy_vld_out <axiReadRespSt<D, RU>> m_data_out; // for sending data (axi_in)
    rdy_vld_in <axiReadAddressSt<A, ARU>> m_addr_in; // for receiving address (axi_in)
    rdy_vld_in <axiReadRespSt<D, RU>> m_data_in; // for receiving data (axi_out)
...
private:
    ...
    // disabled
    axi_read_channel( const axi_read_channel<A, D, ARU, RU>& );
    axi_read_channel& operator = ( const axi_read_channel<A, D, ARU, RU>& );
    std::shared_ptr<stringPingPong> m_logQueueAddr;
    std::shared_ptr<stringPingPong> m_logQueueData;
    void manageSendDataTransaction(const axiReadRespSt<D, RU> & data_);
    void manageReceiveDataTransaction(const axiReadRespSt<D, RU> & data_);
    void handleSendTransactionLog(_axiIdT id);
};

template <class A, class D, class ARU, class RU>
inline void axi_read_channel<A, D, ARU, RU>::register_port( sc_port_base& port_,
                const char* if_typename_ )
{
    std::string nm( if_typename_ );
    if( nm == typeid( axi_read_in_if<A, D, ARU, RU> ).name() )
    {
        ...
        m_reader = &port_;
    } else if( nm == typeid( axi_read_out_if<A, D, ARU, RU> ).name() )
    {
...
template <class A, class D, class ARU, class RU>
inline void axi_read_channel<A, D, ARU, RU>::manageSendDataTransaction(const axiReadRespSt<D, RU> & data_)
{
...
template <class A, class D, class ARU, class RU>
inline void axi_read_channel<A, D, ARU, RU>::manageReceiveDataTransaction(const axiReadRespSt<D, RU> & data_)
{
...
template <class A, class D, class ARU, class RU>
inline void axi_read_channel<A, D, ARU, RU>::handleSendTransactionLog(_axiIdT id)
{
...
template <class A, class D, class ARU, class RU>
inline void axi_read_channel<A, D, ARU, RU>::receiveAddr( axiReadAddressSt<A, ARU> &addr_ )
{
    m_addr_in->read(addr_);
}

template <class A, class D, class ARU, class RU>
inline void axi_read_channel<A, D, ARU, RU>::receiveData( axiReadRespSt<D, RU> &data_ )
{
...
template <class A, class D, class ARU, class RU>
inline uint8_t * axi_read_channel<A, D, ARU, RU>::getReadPtr()
{
...
template <class A, class D, class ARU, class RU>
inline uint8_t * axi_read_channel<A, D, ARU, RU>::getWritePtr()
{
...
template <class A, class D, class ARU, class RU>
void axi_read_channel<A, D, ARU, RU>::receiveDataCycle(  axiReadRespSt<D, RU> & data_)
{
...
template <class A, class D, class ARU, class RU>
inline void axi_read_channel<A, D, ARU, RU>::sendAddr( const axiReadAddressSt<A, ARU>& addr_, std::optional<std::string> str)
{
...
template <class A, class D, class ARU, class RU>
void axi_read_channel<A, D, ARU, RU>::sendData( const axiReadRespSt<D, RU>& data_ )
{
    Q_ASSERT(!m_cycle_transaction, "receiveData not valid api for cycle based transactions");
...
template <class A, class D, class ARU, class RU>
void axi_read_channel<A, D, ARU, RU>::sendData( const axiReadRespSt<D, RU>& data_, int burst_count )
{
    Q_ASSERT(!m_cycle_transaction, "receiveData not valid api for cycle based transactions");
    manageSendDataTransaction(data_);
    handleSendTransactionLog(data_.rid);
    m_data_out->write(data_, burst_count * sizeof(axiReadRespSt<D, RU>));
}


template <class A, class D, class ARU, class RU>
inline void axi_read_channel<A, D, ARU, RU>::sendDataCycle( const axiReadRespSt<D, RU>& data_)
{
    Q_ASSERT(m_cycle_transaction, "receiveDataCycle not valid api for non cycle based transactions, make sure you setCycleTransaction(PORTTYPE_IN)");
...
template <class A, class D, class ARU, class RU>
void axi_read_channel<A, D, ARU, RU>::status(void)
{
...
template <class A, class D, class ARU, class RU>
inline void axi_read_channel<A, D, ARU, RU>::trace( sc_trace_file* tf ) const
{
...
template <class A, class D, class ARU, class RU>
inline void axi_read_channel<A, D, ARU, RU>::print( ::std::ostream& os ) const
{
...
template <class A, class D, class ARU, class RU>
inline void axi_read_channel<A, D, ARU, RU>::dump( ::std::ostream& os ) const
{

}

template <class A, class D, class ARU, class RU>
inline ::std::ostream& operator << ( ::std::ostream& os, const axi_read_channel<A, D, ARU, RU>& a )
{
    a.print( os );
    return os;
}
...
template <class A, class D, class ARU = std::monostate, class RU = std::monostate>
using axi_read_out = sc_port<axi_read_out_if<A, D, ARU, RU> >;
template <class A, class D, class ARU = std::monostate, class RU = std::monostate>
using axi_read_in = sc_port<axi_read_in_if<A, D, ARU, RU> >;
