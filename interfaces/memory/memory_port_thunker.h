...
#ifndef MEMORY_PORT_THUNKER_H
#define MEMORY_PORT_THUNKER_H
...
// class. Bridges an upstream memory consumer-side endpoint carrying (UpA,
// UpD) to a downstream memory consumer port carrying (DownA, DownD), when
// the two pairs are per-field _bitWidth equivalent but differ in nested
// _packedSt width. Per-field equivalence is validated elsewhere; this class
// performs the runtime payload bridge via copyPayload() in both directions
// and preserves the memory request / completion handshake at both ends.
//
...
// below flows Down -> Up).
//
// memory handshake notes (see memory_channel.h):
...
//     transactions the thunker bridges the response data and calls
//     complete() with the converted data.
...
//     response is in dataInOut for reads, and returns after posting the
//     write for writes.
//
// Four construction shapes are supported, spanning a 2x2 family: the child
// (down) end is either a consumer (memory_in<DownA, DownD>&) or a producer
// (memory_out<DownA, DownD>&), and the parent (up) end is either a
...
// end's port type (memory_in vs memory_out), so the container generator
...
class memory_port_thunker
{
public:
    // connectionMap shape: parent port reference.
    memory_port_thunker( const char* name_,
                         memory_in<UpA, UpD>&     upPort,
                         memory_in<DownA, DownD>& downPort,
                         std::string block_ )
      : m_up_port( &upPort ),
    ...
    memory_port_thunker( const char* name_,
                         memory_in_if<UpA, UpD>&  upInIface,
                         memory_in<DownA, DownD>& downPort,
                         std::string block_ )
      : m_up_port( nullptr ),
    ...
    // the parent-side channel's memory_out_if<UpA, UpD>.
    memory_port_thunker( const char* name_,
                         memory_out_if<UpA, UpD>&  upOutIface,
                         memory_out<DownA, DownD>& downPort,
                         std::string block_ )
      : m_up_port( nullptr ),
        m_up_in_iface( nullptr ),
...
private:
    void thunkIn()
    {
        ...
        while (true) {
            ...
            if (!isWrite) {
                ...
                // already acknowledged by memory_channel::reqReceive() —
    ...
    void thunkOut()
    {
        // Producer (out) shape: the child requester drives DownA/DownD
        ...
        // every Up<->Down type swapped, preserving the same completion
    ...
    memory_in<UpA, UpD>*     m_up_port;
    memory_in_if<UpA, UpD>*  m_up_in_iface;
    memory_out_if<UpA, UpD>* m_up_out_iface;
    memory_out<UpA, UpD>*    m_up_out_port;
    memory_channel<DownA, DownD> m_down_channel;
};

#endif // MEMORY_PORT_THUNKER_H
