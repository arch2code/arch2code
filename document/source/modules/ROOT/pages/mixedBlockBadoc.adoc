...
|btod |req_ack |blockB is Destination| data_t |xref:#aSt[aSt]| An interface for A
|||| rdata_t |xref:#aASt[aASt]|
|startDone |notify_ack |blockB is Destination| None| None| A start done interface
|apbReg |apb |blockB is Destination| addr_t |xref:#apbAddrSt[apbAddrSt]| CPU access to SoC registers in the design
|||| data_t |xref:#apbDataSt[apbDataSt]|
|===
