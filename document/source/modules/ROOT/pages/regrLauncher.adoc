...
==== Substitution variables in run commands ====
The following variables are expanded in ``command`` and ``args`` attributes when each run job is scheduled:

* ``RL_SEED`` -- value of the run command ``seed`` attribute for the current job (see example below)
* ``RL_RUN_ID`` -- unique run identifier for the current job (``run_0``, ``run_1``, ...), matching the ``run_<rid>`` directory created under the session ``runs/`` tree

[source, text]
"command" : "echo 'my seed is ' :",
"args" : "${RL_SEED}"

For cocotb-based run commands, the runner sets ``COCOTB_RESULTS_FILE`` to ``results.xml`` in each job's ``run_<rid>`` directory so parallel jobs do not share a results file under the testbench tree.
