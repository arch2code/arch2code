...
----
{
  ...
  "build" : {
    ...
    "command+" : <build_command:string|list:optional>,
...
* Non ``string`` or ``list`` attribute types (such as integer) do not support the ``+`` (append) flag
* In the ``build`` container the ``+`` flag appends to the attribute of the same name in that container, since there is no parent. A list value is joined with spaces, so a long build command can be written one entry per line
...
|build.command+ |string, list of strings |Appended to build.command after a space. A list is joined with spaces, one entry per line for readability, for example one ``DUT_TOPOLOGIES+=<inst>:<cfg>,<cfg>...`` entry per block for the simulator snapshot flows. +
``--attr build.command+=<value>`` appends after the ``command+`` entries and ``--attr build.command=<value>`` replaces the resolved command, entries included. ``--attr`` splits its value on commas, so a comma-separated entry such as the topology example must be passed one configuration per ``--attr`` occurrence
