...
. Clocks and resets will be abbreviated to `clk` and `rst`, `rst_n` for active low.
...
. Another abbreviation that will always be present are the signals for clock and reset. They will be `clk` and `rst_n`. This is the main clock and active low reset.
