...
// GENERATED_CODE_PARAM --block=xpCstUseTop
...
class xpCstUseTopConfig : public testBenchConfigBase
{
public:
    virtual ~xpCstUseTopConfig() override = default; // Explicit Virtual Destructor
...
protected:
    ...
    std::shared_ptr<blockBase> createTbTop(void) { return instanceFactory::createInstance("", "tb", "xpCstUseTopTestbench", "", "xpCstUse"); }
public:
// GENERATED_CODE_END
...
// === Testbench config registration (xpCstUseTopConfig) ===
...
void register_xpCstUseTopConfig() {
    testBenchConfigFactory::registerTestBenchConfig("xpCstUseTop", [](std::string) -> std::shared_ptr<testBenchConfigBase> { return static_cast<std::shared_ptr<testBenchConfigBase>> (std::make_shared<xpCstUseTopConfig>());}, is_default_testbench_v<xpCstUseTopConfig>);
}

namespace {
[[maybe_unused]] A2C_REGISTRATION_RETAIN int _xpCstUseTopConfig_registered = (register_xpCstUseTopConfig(), 0);
} // namespace
