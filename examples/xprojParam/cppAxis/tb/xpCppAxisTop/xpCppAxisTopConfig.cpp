...
// GENERATED_CODE_PARAM --block=xpCppAxisTop
...
class xpCppAxisTopConfig : public testBenchConfigBase
{
public:
    virtual ~xpCppAxisTopConfig() override = default; // Explicit Virtual Destructor
...
protected:
    ...
    std::shared_ptr<blockBase> createTbTop(void) { return instanceFactory::createInstance("", "tb", "xpCppAxisTopTestbench", "", "xpCppAxis"); }
public:
// GENERATED_CODE_END
...
// === Testbench config registration (xpCppAxisTopConfig) ===
...
void register_xpCppAxisTopConfig() {
    testBenchConfigFactory::registerTestBenchConfig("xpCppAxisTop", [](std::string) -> std::shared_ptr<testBenchConfigBase> { return static_cast<std::shared_ptr<testBenchConfigBase>> (std::make_shared<xpCppAxisTopConfig>());}, is_default_testbench_v<xpCppAxisTopConfig>);
}

namespace {
[[maybe_unused]] A2C_REGISTRATION_RETAIN int _xpCppAxisTopConfig_registered = (register_xpCppAxisTopConfig(), 0);
} // namespace
