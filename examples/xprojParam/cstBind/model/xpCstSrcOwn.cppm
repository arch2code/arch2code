...
export module xpCstBind_xpCstSrcOwn.block;
...
SC_MODULE(xpCstSrcOwn), public blockBase, public xpCstSrcOwnBase<Config>
{
private:

public:
    SC_HAS_PROCESS(xpCstSrcOwn);

    // inherited names usable unqualified (no Config:: / this->)
    using xpCstSrcOwnBase<Config>::CS_OWN_WIDTH;
    using xpCstSrcOwnBase<Config>::out;


    // inherited parameterized types usable unqualified (no <Config>)
    using typename xpCstSrcOwnBase<Config>::csOwnPixelT;
    using typename xpCstSrcOwnBase<Config>::csOwnSt;

    xpCstSrcOwn(sc_module_name blockName, const char * variant, blockBaseMode bbMode);
    ~xpCstSrcOwn() override = default;
...
xpCstSrcOwn<Config>::xpCstSrcOwn(sc_module_name blockName, const char * variant, blockBaseMode bbMode)
       : sc_module(blockName)
        ,blockBase("xpCstSrcOwn", name(), bbMode)
        ,xpCstSrcOwnBase<Config>(name(), variant)
// GENERATED_CODE_END
...
template<typename Config>
void xpCstSrcOwn<Config>::drive(void)
{
    Q_ASSERT(CS_OWN_WIDTH == expectedWidth(),
        std::format("xpCstSrcOwn variant '{}' must resolve CS_OWN_WIDTH to {} but resolved {}",
            m_variant, expectedWidth(), (uint64_t)CS_OWN_WIDTH));
    for (uint32_t i = 0; i < SAMPLE_COUNT; i++) {
        csOwnSt sample{};
        sample.tag  = FIRST_TAG + i;
        sample.cfg  = CS_OWN_WIDTH;
