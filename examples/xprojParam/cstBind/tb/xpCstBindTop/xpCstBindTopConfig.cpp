...
// GENERATED_CODE_PARAM --block=xpCstBindTop
...
class xpCstBindTopConfig : public testBenchConfigBase
{
public:
    virtual ~xpCstBindTopConfig() override = default; // Explicit Virtual Destructor
...
protected:
    ...
    std::shared_ptr<blockBase> createTbTop(void) { return instanceFactory::createInstance("", "tb", "xpCstBindTopTestbench", "", "xpCstBind"); }
public:
// GENERATED_CODE_END
...
// === Testbench config registration (xpCstBindTopConfig) ===
...
void register_xpCstBindTopConfig() {
    testBenchConfigFactory::registerTestBenchConfig("xpCstBindTop", [](std::string) -> std::shared_ptr<testBenchConfigBase> { return static_cast<std::shared_ptr<testBenchConfigBase>> (std::make_shared<xpCstBindTopConfig>());}, is_default_testbench_v<xpCstBindTopConfig>);
}

namespace {
[[maybe_unused]] A2C_REGISTRATION_RETAIN int _xpCstBindTopConfig_registered = (register_xpCstBindTopConfig(), 0);
} // namespace
