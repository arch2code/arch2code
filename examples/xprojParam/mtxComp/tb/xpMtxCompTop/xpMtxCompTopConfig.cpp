...
// GENERATED_CODE_PARAM --block=xpMtxCompTop
...
class xpMtxCompTopConfig : public testBenchConfigBase
{
public:
    virtual ~xpMtxCompTopConfig() override = default; // Explicit Virtual Destructor
...
protected:
    ...
    std::shared_ptr<blockBase> createTbTop(void) { return instanceFactory::createInstance("", "tb", "xpMtxCompTopTestbench", "", "xpMtxComp"); }
public:
// GENERATED_CODE_END
...
// === Testbench config registration (xpMtxCompTopConfig) ===
...
void register_xpMtxCompTopConfig() {
    testBenchConfigFactory::registerTestBenchConfig("xpMtxCompTop", [](std::string) -> std::shared_ptr<testBenchConfigBase> { return static_cast<std::shared_ptr<testBenchConfigBase>> (std::make_shared<xpMtxCompTopConfig>());}, is_default_testbench_v<xpMtxCompTopConfig>);
}

namespace {
[[maybe_unused]] A2C_REGISTRATION_RETAIN int _xpMtxCompTopConfig_registered = (register_xpMtxCompTopConfig(), 0);
} // namespace
