...
// GENERATED_CODE_PARAM --block=xpDpTop
...
class xpDpTopConfig : public testBenchConfigBase
{
public:
    virtual ~xpDpTopConfig() override = default; // Explicit Virtual Destructor
...
protected:
    ...
    std::shared_ptr<blockBase> createTbTop(void) { return instanceFactory::createInstance("", "tb", "xpDpTopTestbench", "", "xpDpTop"); }
public:
// GENERATED_CODE_END
...
// === Testbench config registration (xpDpTopConfig) ===
...
void register_xpDpTopConfig() {
    testBenchConfigFactory::registerTestBenchConfig("xpDpTop", [](std::string) -> std::shared_ptr<testBenchConfigBase> { return static_cast<std::shared_ptr<testBenchConfigBase>> (std::make_shared<xpDpTopConfig>());}, is_default_testbench_v<xpDpTopConfig>);
}

namespace {
[[maybe_unused]] A2C_REGISTRATION_RETAIN int _xpDpTopConfig_registered = (register_xpDpTopConfig(), 0);
} // namespace
