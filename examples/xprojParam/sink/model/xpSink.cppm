...
SC_MODULE(xpSink), public blockBase, public xpSinkBase<Config>
{
private:

public:
    SC_HAS_PROCESS(xpSink);

    // inherited names usable unqualified (no Config:: / this->)
    using xpSinkBase<Config>::PIXEL_WIDTH;
    using xpSinkBase<Config>::videoIn;


    // inherited parameterized types usable unqualified (no <Config>)
    using typename xpSinkBase<Config>::pixel_t;
    using typename xpSinkBase<Config>::videoSt;

    xpSink(sc_module_name blockName, const char * variant, blockBaseMode bbMode);
    ~xpSink() override = default;
...
xpSink<Config>::xpSink(sc_module_name blockName, const char * variant, blockBaseMode bbMode)
       : sc_module(blockName)
        ,blockBase("xpSink", name(), bbMode)
        ,xpSinkBase<Config>(name(), variant)
// GENERATED_CODE_END
...
template<typename Config>
void xpSink<Config>::checkVideo(void)
{
    Q_ASSERT(PIXEL_WIDTH == 8, "xpSink PIXEL_WIDTH must resolve to the bound variant value 8");
    m_eot.registerVoter();
    videoSt sample;
