...
// GENERATED_CODE_PARAM --block=xpInhTop
...
class xpInhTopConfig : public testBenchConfigBase
{
public:
    virtual ~xpInhTopConfig() override = default; // Explicit Virtual Destructor
...
protected:
    ...
    std::shared_ptr<blockBase> createTbTop(void) { return instanceFactory::createInstance("", "tb", "xpInhTopTestbench", "", "xpInhVar"); }
public:
// GENERATED_CODE_END
...
// === Testbench config registration (xpInhTopConfig) ===
...
void register_xpInhTopConfig() {
    testBenchConfigFactory::registerTestBenchConfig("xpInhTop", [](std::string) -> std::shared_ptr<testBenchConfigBase> { return static_cast<std::shared_ptr<testBenchConfigBase>> (std::make_shared<xpInhTopConfig>());}, is_default_testbench_v<xpInhTopConfig>);
}

namespace {
[[maybe_unused]] A2C_REGISTRATION_RETAIN int _xpInhTopConfig_registered = (register_xpInhTopConfig(), 0);
} // namespace
