...
class xpMtxTplTopConfig : public testBenchConfigBase
{
public:
    virtual ~xpMtxTplTopConfig() override = default; // Explicit Virtual Destructor
...
protected:
    ...
    std::shared_ptr<blockBase> createTbTop(void) { return instanceFactory::createInstance("", "tb", "xpMtxTplTopTestbench", "", "xpMtxTpl"); }
public:
// GENERATED_CODE_END
...
// === Testbench config registration (xpMtxTplTopConfig) ===
...
void register_xpMtxTplTopConfig() {
    testBenchConfigFactory::registerTestBenchConfig("xpMtxTplTop", [](std::string) -> std::shared_ptr<testBenchConfigBase> { return static_cast<std::shared_ptr<testBenchConfigBase>> (std::make_shared<xpMtxTplTopConfig>());}, is_default_testbench_v<xpMtxTplTopConfig>);
}

namespace {
[[maybe_unused]] A2C_REGISTRATION_RETAIN int _xpMtxTplTopConfig_registered = (register_xpMtxTplTopConfig(), 0);
} // namespace
