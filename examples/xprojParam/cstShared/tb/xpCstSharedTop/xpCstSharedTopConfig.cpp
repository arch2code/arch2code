...
// GENERATED_CODE_PARAM --block=xpCstSharedTop
...
class xpCstSharedTopConfig : public testBenchConfigBase
{
public:
    virtual ~xpCstSharedTopConfig() override = default; // Explicit Virtual Destructor
...
protected:
    ...
    std::shared_ptr<blockBase> createTbTop(void) { return instanceFactory::createInstance("", "tb", "xpCstSharedTopTestbench", "", "xpCstShared"); }
public:
// GENERATED_CODE_END
...
// === Testbench config registration (xpCstSharedTopConfig) ===
...
void register_xpCstSharedTopConfig() {
    testBenchConfigFactory::registerTestBenchConfig("xpCstSharedTop", [](std::string) -> std::shared_ptr<testBenchConfigBase> { return static_cast<std::shared_ptr<testBenchConfigBase>> (std::make_shared<xpCstSharedTopConfig>());}, is_default_testbench_v<xpCstSharedTopConfig>);
}

namespace {
[[maybe_unused]] A2C_REGISTRATION_RETAIN int _xpCstSharedTopConfig_registered = (register_xpCstSharedTopConfig(), 0);
} // namespace
