...
// GENERATED_CODE_PARAM --block=xpSharedTop
...
class xpSharedTopConfig : public testBenchConfigBase
{
public:
    virtual ~xpSharedTopConfig() override = default; // Explicit Virtual Destructor
...
protected:
    ...
    std::shared_ptr<blockBase> createTbTop(void) { return instanceFactory::createInstance("", "tb", "xpSharedTopTestbench", "", "xpShared"); }
public:
// GENERATED_CODE_END
...
// === Testbench config registration (xpSharedTopConfig) ===
...
void register_xpSharedTopConfig() {
    testBenchConfigFactory::registerTestBenchConfig("xpSharedTop", [](std::string) -> std::shared_ptr<testBenchConfigBase> { return static_cast<std::shared_ptr<testBenchConfigBase>> (std::make_shared<xpSharedTopConfig>());}, is_default_testbench_v<xpSharedTopConfig>);
}

namespace {
[[maybe_unused]] A2C_REGISTRATION_RETAIN int _xpSharedTopConfig_registered = (register_xpSharedTopConfig(), 0);
} // namespace
