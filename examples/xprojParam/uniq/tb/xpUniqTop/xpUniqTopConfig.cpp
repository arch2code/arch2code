...
// GENERATED_CODE_PARAM --block=xpUniqTop
...
class xpUniqTopConfig : public testBenchConfigBase
{
public:
    virtual ~xpUniqTopConfig() override = default; // Explicit Virtual Destructor
...
protected:
    ...
    std::shared_ptr<blockBase> createTbTop(void) { return instanceFactory::createInstance("", "tb", "xpUniqTopTestbench", "", "xpUniq"); }
public:
// GENERATED_CODE_END
...
// === Testbench config registration (xpUniqTopConfig) ===
...
void register_xpUniqTopConfig() {
    testBenchConfigFactory::registerTestBenchConfig("xpUniqTop", [](std::string) -> std::shared_ptr<testBenchConfigBase> { return static_cast<std::shared_ptr<testBenchConfigBase>> (std::make_shared<xpUniqTopConfig>());}, is_default_testbench_v<xpUniqTopConfig>);
}

namespace {
[[maybe_unused]] A2C_REGISTRATION_RETAIN int _xpUniqTopConfig_registered = (register_xpUniqTopConfig(), 0);
} // namespace
