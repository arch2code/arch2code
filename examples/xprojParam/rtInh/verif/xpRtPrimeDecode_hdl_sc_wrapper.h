#ifndef XPRTPRIMEDECODE_HDL_SC_WRAPPER_H_
#define XPRTPRIMEDECODE_HDL_SC_WRAPPER_H_

// GENERATED_CODE_PARAM --block=xpRtPrimeDecode
// GENERATED_CODE_BEGIN --template=module_hdl_sc_wrapper --section=preamble
#include "systemc.h"
#include "blockBase.h"
import xpRtInh_xpRtPrimeDecode.base;

// A non-templated wrapper names its Verilated RTL top concretely, so it
// includes the DUT header directly.
#if !defined(VERILATOR) && defined(VCS)
#include "xpRtPrimeDecode_hdl_sv_wrapper.h"
#else
#include "VxpRtPrimeDecode_hdl_sv_wrapper.h"
#endif
// GENERATED_CODE_END

// GENERATED_CODE_BEGIN --template=module_hdl_sc_wrapper --section=hdl_sc_wrapper_class

import common_shared_types;
using namespace common_shared_types_ns;
#include "apb_bfm.h"

class xpRtPrimeDecode_hdl_sc_wrapper: public sc_module, public blockBase, public xpRtPrimeDecodeBase {

public:

#if !defined(VERILATOR) && defined(VCS)
    xpRtPrimeDecode_hdl_sv_wrapper *dut_hdl;
#else
    VxpRtPrimeDecode_hdl_sv_wrapper *dut_hdl;
#endif

    sc_clock clk;

    apb_src_bfm<apbAddrSt, apbDataSt, sc_bv<32>, sc_bv<32>> apbReg_uWrap_bfm;
    apb_dst_bfm<apbAddrSt, apbDataSt, sc_bv<32>, sc_bv<32>> cpu_main_bfm;

    SC_HAS_PROCESS (xpRtPrimeDecode_hdl_sc_wrapper);

    xpRtPrimeDecode_hdl_sc_wrapper(sc_module_name modulename, const char *variant, blockBaseMode bbMode) :
        sc_module(modulename),
        blockBase("xpRtPrimeDecode_hdl_sc_wrapper", name(), bbMode),
        xpRtPrimeDecodeBase(name(), variant),
        clk("clk", sc_time(1, SC_NS), 0.5, sc_time(3, SC_NS), true),
        apbReg_uWrap_bfm("apbReg_uWrap_bfm"),
        cpu_main_bfm("cpu_main_bfm"),
        rst_n(0)
    {
#if !defined(VERILATOR) && defined(VCS)
        dut_hdl = new xpRtPrimeDecode_hdl_sv_wrapper("dut_hdl");
#else
        dut_hdl = new VxpRtPrimeDecode_hdl_sv_wrapper("dut_hdl");
#endif

        dut_hdl->apbReg_uWrap_paddr(apbReg_uWrap_hdl_if.paddr);
        dut_hdl->apbReg_uWrap_psel(apbReg_uWrap_hdl_if.psel);
        dut_hdl->apbReg_uWrap_penable(apbReg_uWrap_hdl_if.penable);
        dut_hdl->apbReg_uWrap_pwrite(apbReg_uWrap_hdl_if.pwrite);
        dut_hdl->apbReg_uWrap_pwdata(apbReg_uWrap_hdl_if.pwdata);
        dut_hdl->apbReg_uWrap_pready(apbReg_uWrap_hdl_if.pready);
        dut_hdl->apbReg_uWrap_prdata(apbReg_uWrap_hdl_if.prdata);
        dut_hdl->apbReg_uWrap_pslverr(apbReg_uWrap_hdl_if.pslverr);
        dut_hdl->cpu_main_paddr(cpu_main_hdl_if.paddr);
        dut_hdl->cpu_main_psel(cpu_main_hdl_if.psel);
        dut_hdl->cpu_main_penable(cpu_main_hdl_if.penable);
        dut_hdl->cpu_main_pwrite(cpu_main_hdl_if.pwrite);
        dut_hdl->cpu_main_pwdata(cpu_main_hdl_if.pwdata);
        dut_hdl->cpu_main_pready(cpu_main_hdl_if.pready);
        dut_hdl->cpu_main_prdata(cpu_main_hdl_if.prdata);
        dut_hdl->cpu_main_pslverr(cpu_main_hdl_if.pslverr);
        dut_hdl->clk(clk);
        dut_hdl->rst_n(rst_n);

        apbReg_uWrap_bfm.if_p(this->apbReg_uWrap);
        apbReg_uWrap_bfm.hdl_if_p(apbReg_uWrap_hdl_if);
        apbReg_uWrap_bfm.clk(clk);
        apbReg_uWrap_bfm.rst_n(rst_n);

        cpu_main_bfm.if_p(this->cpu_main);
        cpu_main_bfm.hdl_if_p(cpu_main_hdl_if);
        cpu_main_bfm.clk(clk);
        cpu_main_bfm.rst_n(rst_n);

        SC_THREAD(reset_driver);

        end_ctor_init();

    }

public:

#ifdef VERILATOR
    void vl_trace(VerilatedVcdC* tfp, int levels, int options = 0) override {
        dut_hdl->trace(tfp, levels, options);
    }
#endif

private:

    apb_hdl_if<sc_bv<32>, sc_bv<32>> apbReg_uWrap_hdl_if;
    apb_hdl_if<sc_bv<32>, sc_bv<32>> cpu_main_hdl_if;

    sc_signal<bool> rst_n;

    void reset_driver() {
        wait(5, SC_NS);
        rst_n = true;
    }

// GENERATED_CODE_END

    // Callback executed at the end of module constructor
    void end_ctor_init() {
        // Register synchLock,...
    }

};

#endif // XPRTPRIMEDECODE_HDL_SC_WRAPPER_H_
