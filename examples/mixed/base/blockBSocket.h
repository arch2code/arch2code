...
private:
    struct registerBlock
    {
        registerBlock()
        {
            ...
            instanceFactory::registerBlock("blockB_socket", [](const char * blockName, const char * variant, blockBaseMode bbMode) -> std::shared_ptr<blockBase> { return static_cast<std::shared_ptr<blockBase>> (std::make_shared<blockBSocket>(blockName, variant, bbMode));}, "", "mixed");
        }
    };
