...
private:
    struct registerBlock
    {
        registerBlock()
        {
            ...
            instanceFactory::registerBlock("apbDecode_socket", [](const char * blockName, const char * variant, blockBaseMode bbMode) -> std::shared_ptr<blockBase> { return static_cast<std::shared_ptr<blockBase>> (std::make_shared<apbDecodeSocket>(blockName, variant, bbMode));}, "", "simple_ip");
        }
    };
