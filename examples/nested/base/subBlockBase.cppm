//copyright the arch2code project contributors, see https://github.com/arch2code/arch2code/blob/main/LICENSE

// GENERATED_CODE_PARAM --block=subBlock --mode=module
// GENERATED_CODE_BEGIN --template=moduleScaffold --section=baseModuleHeader
module;
#include "systemc.h"
#include "rdy_vld_channel.h"

export module nested_subBlock.base;
import nested;
using namespace nested_ns;
// GENERATED_CODE_END

// GENERATED_CODE_BEGIN --template=baseClassDecl

export class subBlockBase : public virtual blockPortBase
{
public:
    virtual ~subBlockBase() = default;
    // src ports
    // test->uSubBlock1: Test interface
    rdy_vld_out< test_st > src;

    // dst ports
    // uSubBlock0->test: Test interface
    rdy_vld_in< test_st > dst;


    subBlockBase(std::string name, const char * variant) :
        src("src")
        ,dst("dst")
    {};
    void setTimed(int nsec, timedDelayMode mode) override
    {
        src->setTimed(nsec, mode);
        dst->setTimed(nsec, mode);
        setTimedLocal(nsec, mode);
    };
    void setLogging(verbosity_e verbosity) override
    {
        src->setLogging(verbosity);
        dst->setLogging(verbosity);
    };
};
export class subBlockInverted : public virtual blockPortBase
{
public:
    // src ports
    // test->uSubBlock1: Test interface
    rdy_vld_in< test_st > src;

    // dst ports
    // uSubBlock0->test: Test interface
    rdy_vld_out< test_st > dst;


    subBlockInverted(std::string name) :
        src(("src"+name).c_str())
        ,dst(("dst"+name).c_str())
    {};
    void setTimed(int nsec, timedDelayMode mode) override
    {
        src->setTimed(nsec, mode);
        dst->setTimed(nsec, mode);
        setTimedLocal(nsec, mode);
    };
    void setLogging(verbosity_e verbosity) override
    {
        src->setLogging(verbosity);
        dst->setLogging(verbosity);
    };
};
export class subBlockChannels
{
public:
    // src ports
    // Test interface
    rdy_vld_channel< test_st > src;

    // dst ports
    // Test interface
    rdy_vld_channel< test_st > dst;


    subBlockChannels(std::string name, std::string srcName) :
    src(("src"+name).c_str(), srcName)
    ,dst(("dst"+name).c_str(), srcName)
    {};
    void bind( subBlockBase *a, subBlockInverted *b)
    {
        a->src( src );
        b->src( src );
        a->dst( dst );
        b->dst( dst );
    };
};

// GENERATED_CODE_END
