...
private:
    struct registerBlock
    {
        registerBlock(const char * variant_)
        {
            ...
            instanceFactory::registerBlock("src_socket", [](const char * blockName, const char * variant, blockBaseMode bbMode) -> std::shared_ptr<blockBase> { return static_cast<std::shared_ptr<blockBase>> (std::make_shared<srcSocket<Config>>(blockName, variant, bbMode));}, variant_, "ip_test");
        }
    };
...
private:
    ...
    void out1Socket(void);
    void out2Socket(void);
    void out3Socket(void);
