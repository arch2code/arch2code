...
bool fwCheckUIp0(void)
{
    ...
    uint32_t lastData = regRead32(BASE_ADDR_UIP0 + REG_IP_IPLASTDATA);
    ...
    regWrite32(BASE_ADDR_UIP0 + REG_IP_IPCFG, pattern);
    uint32_t readback = regRead32(BASE_ADDR_UIP0 + REG_IP_IPCFG);
...
bool fwCheckUIp1(void)
{
    ...
    regWrite32(BASE_ADDR_UIP1 + REG_IP_IPCFG, pattern);
    uint32_t readback = regRead32(BASE_ADDR_UIP1 + REG_IP_IPCFG);
    return (readback & 0xFF) == (pattern & 0xFF);
}
