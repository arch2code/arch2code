...
private:
    struct registerBlock
    {
        registerBlock()
        {
            ...
            instanceFactory::registerBlock("axi4s_m_drv_socket", [](const char * blockName, const char * variant, blockBaseMode bbMode) -> std::shared_ptr<blockBase> { return static_cast<std::shared_ptr<blockBase>> (std::make_shared<axi4s_m_drvSocket>(blockName, variant, bbMode));}, "", "hierVlDemo");
        }
    };
