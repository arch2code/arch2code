...
export module axiSocketMaster_axiSocket.testbench;
import axiSocketMaster_axiSocket.base;
import axiSocketMaster_axiSocket.external;
...
void register_axiSocketTestbench_variants() {
    instanceFactory::registerBlock("axiSocketTestbench_model", [](const char * blockName, const char * variant, blockBaseMode bbMode) -> std::shared_ptr<blockBase> { return static_cast<std::shared_ptr<blockBase>>(std::make_shared<axiSocketTestbench>(blockName, variant, bbMode)); }, "", "axiSocketMaster");
}
...
axiSocketTestbench::axiSocketTestbench(sc_module_name blockName, const char * variant, blockBaseMode bbMode)
       : blockBase("axiSocketTestbench", name(), bbMode)
        ...
        ,axiSocket(std::dynamic_pointer_cast<axiSocketBase>( instanceFactory::createInstance(name(), "axiSocket", "axiSocket", "", "axiSocketMaster")))
        ,external("external")
{
