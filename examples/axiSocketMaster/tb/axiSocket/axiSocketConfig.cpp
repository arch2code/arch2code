...
std::string resolveAxiSocketMasterScriptPath()
{
    ...
    const std::string candidate = build_dir + "/../../axiSocketMaster.py";
    char resolved[PATH_MAX];
...
protected:
    ...
    std::shared_ptr<blockBase> createTbTop(void) { return instanceFactory::createInstance("", "tb", "axiSocketTestbench", "", "axiSocketMaster"); }
public:
// GENERATED_CODE_END
...
public:
    bool createTestBench(void) override
    {
        ...
        if (!skip_sidecar_) {
            const std::string script_path = resolveAxiSocketMasterScriptPath();
            if (script_path.empty()) {
                socketFactory::shutdownAll();
                return false;
            }
            pid_t sc_pid = getpid();
            sc_ports_file = "/tmp/axi_socket_master_ports_" + std::to_string(sc_pid) + ".env";
        ...
        (void)instanceFactory::createInstance("", "axiSocketMaster_tb", "axiSocketMaster_tb", "", "axiSocketMaster");
        return true;
    }
