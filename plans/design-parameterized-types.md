...
- A single migration command, `make migrate`, performs all of the above conversions. It stops short of floor-division evals. Python `//` floors toward -inf where SV `/` truncates toward zero, so every `//` is reported for hand rewrite rather than converted.
...
Every `project.yaml` must declare `yamlFormat: 2` as a top-level field, or the generator hard-stops before parsing any YAML (`CURRENT_YAML_FORMAT = 2` in `pysrc/processYaml.py`; gate in `_gateYamlFormat`). Verified present in `prj/yaml/debayerProject.yaml` and `builder/base/examples/ip_test/prj/yaml/ip_testProject.yaml`.

#### eval expressions: SystemVerilog subset, not Python

`eval:` strings are parsed by a frozen SystemVerilog constant-expression subset. The debayer migration shows both outcomes, the automatic rewrite and the one that stops for a human:

```yaml
# CONVERTED - rewritten in place by migration Phase A
VERTICAL_SIZE_LOG2: {eval: "($VERTICAL_SIZE-1).bit_length()"} → {eval: "$clog2($VERTICAL_SIZE)"}

# NEEDS_MANUAL - reported, left untouched, rewritten by hand
DEBAYER_OFFSET:     {eval: "($DEBAYER_DIMENSION - 1) // 2"}
ENTRIES_PER_LINE:   {eval: "$HORIZONTAL_SIZE // $PIXELS_PER_CLOCK"}
```

The converter rewrites two constructs. `.bit_length()` becomes `$clog2(...)`, and a C-style integer literal is re-spelled in SV based form (`0xFF` becomes `'hff`). Already-SV expressions such as `(1 << $BITS_PER_PIXEL_COLOR) - 1` are unchanged. Floor division is never rewritten, because Python `//` floors toward -inf where SV `/` truncates toward zero. Both debayer cases above divide non-negative operands, so a human read them and wrote `/`. Real-valued evals (`/ 2.0`) and `**` are out of grammar and must become literal `value:` entries. Comparisons and the ternary conditional are in the grammar and need no conversion. See §4.2 for the grammar and §4.3 for the converter.

#### Parameterizable types and `ipParameters`

`ipParameters:` is for the block's **root parameter constants** — the variant knobs named in `params:` and bound per variant. These **must live in the IP-root YAML file** (the same file as the block's `params:` endpoint), because that is how the generator identifies them as the variant-bound parameters. Each carries a required worst-case `maxValue` (`builder/base/examples/ip_test/ip/yaml/ip.yaml`):
...
- **`make migrate`** — the single command that brings an existing project to `yamlFormat: 2`. It wraps `migrateYaml.py --write` (`builder/base/include/make/a2c-common.mk`) and is standalone: it rewrites YAML as text and never opens the database, because a pre-migration project cannot pass the gate.
...
The gate's remediation message (`_gateYamlFormat` in `pysrc/processYaml.py`):
...
2. **Phase A — eval Python to SV** — `CONVERTED` rows are rewritten in place; `NEEDS_MANUAL` rows are reported, never rewritten, and block the stamp. Floor division is the common case. Debayer's `DEBAYER_OFFSET` and `ENTRIES_PER_LINE` were both `//` expressions, and both had to be rewritten by hand. Real-valued evals, `**`, non-`bit_length` calls, and booleans land there too.
...
> **Clarification on RTL packages.** Neither SystemVerilog package was **deleted**, though one moved. `rtl/debayer_package.sv` is still at the debayer root. The shared-types package left `rtl/` for the `isp_shared` submodule, which `main` does not carry at all, and now lives at `isp_shared/rtl/isp_types_package.sv`. Their *parameterizable contents* were relocated into module scope; the packages now carry only non-parameterizable items. See §6.2.6.
...
| `eval:` syntax | Python (`.bit_length()`, `//`, `/2.0`) | SV subset (`$clog2`, `/`, comparisons, `?:`) | `.bit_length()` and C-style literals auto-converted; `//`, real, and `**` reported for hand rewrite |
...
1. **Migration gate.** `projectCreate` calls `_gateYamlFormat()` first (`pysrc/processYaml.py`). A project must carry `yamlFormat: 2` or the build hard-stops before any YAML is parsed.
...
`evalExpr.py` is a standalone, project-neutral module owning the tokenizer, an in-house precedence-climbing (Pratt) parser, the IR node classes, the numeric evaluator, and the canonical serializer. It knows nothing about `ValueResolver` or project data — symbol qualification and value lookup are injected as callbacks.

**What an eval expression is.** In YAML a constant's `value:` field is an `eval` field (`config/schema.yaml`). The user may write a literal value or `eval: "<expr>"`. The expression language is a **frozen integer SystemVerilog constant-expression subset**, not Python.

**IR nodes** (frozen dataclasses in `evalExpr.py`): `Num(value, base)`, `Sym(key)`, `Unary(op, operand)`, `Bin(op, lhs, rhs)`, `Clog2(operand)`, `Cond(cond, then, otherwise)`.

**Grammar and literals.**
- Unsized decimal (`255`, `4_095`) and SV based literals: `8'hFF`, `'hFF`, `'b1010`, `12'd4095`, signed `'shFF`, with `_` separators (`_tokenizeNumber` / `_tokenizeBased`).
- Octal is normalized to base-16 at parse time (`_tokenizeBased`), since C has no clean octal spelling.
...
- **Rejected at parse** by the tokenizer: real literals and scientific notation, C-style `0x`/`0o`/`0b` prefixes, `x`/`z`/`?` digits, and a lone `=` or `!`, each diagnosed as a probable `==` or `!=`.

**Operators and precedence** (the `PRECEDENCE` table in `evalExpr.py`; unary `{+, -, ~}`). Higher binds tighter, all binary operators left-associative:

```
|:1   ^:2   &:3   ==,!=:4   <,<=,>,>=:5   <<,>>:6   +,-:7   *,/,%:8
```

The ternary conditional `a ? b : c` is not in that table. It binds looser than every binary operator and is right-associative, parsed as its own layer by `_Parser._ternary`.

`$clog2(...)` is the sole function. Out of grammar: `**`, `~^`/`^~`, arithmetic shifts `<<<`/`>>>`, logical `&&`/`||`/`!`, and `$bits` — so every retained operator has a one-to-one SV and C++ spelling.

**Symbol forms** (`evalExpr.py`):
...
**The Pratt parser** (`_Parser` in `evalExpr.py`). `parse` enters at `_ternary`, which parses a full binary expression through `_expr(0)` and then, on a `?`, recurses into `_ternary` for both branches so conditionals group rightward. `_expr(minPrec)` does the precedence climbing: it parses a unary atom, then while the next operator's precedence is at least `minPrec` it consumes the operator and recurses at `PRECEDENCE[op] + 1` to enforce left-associativity. A parenthesized group and a `$clog2` argument both re-enter at `_ternary`. Parentheses are consumed and discarded; grouping survives only as tree shape.

**Canonical serialization** (`unparse` in `evalExpr.py`): symbols become `${key}`, literals are emitted in unsized SV based form per authored radix, operators single-space-padded, parentheses re-inserted from precedence. The invariant `parse(unparse(node)) == node` holds.

**Evaluation** (`evaluate(node, resolve)` in `evalExpr.py`) follows SV/C++ integer semantics, not Python:
- `/` truncates toward zero (`_truncDiv`), not Python floor `//`.
- `%` follows the truncated quotient.
- Relational and equality operators yield a 0/1 int, and a `Cond` takes its `then` branch on any nonzero condition, matching SV/C++ truthiness.
...
`symbolKeys(node)` returns the set of referenced qualified keys for parameterizable-dependency detection without evaluating.

**Integration in `processYaml.py`** — parse once, evaluate up to twice:
- Parse at ingestion. `EvalParseError` becomes a user diagnostic; the field defaults to 0 to avoid cascades.
- **Active value** via `evaluate(node, resolve=value)`.
- `unparse(node)` persisted to `evalCanonical`.
- **Worst-case maxValue**: if any referenced symbol is parameterizable, the constant is "derived-parameterizable" and the same tree is re-evaluated with `resolve=maxValue`.
...
**Why.** Legacy `eval:` strings were authored in Python and do not parse under the frozen SV grammar (`evalPyToSv.py`).

**What it converts** (`convertExpr`), three outcomes:
- `ALREADY_SV` — input already parses under `evalExpr.parse` (the skip gate; makes it idempotent and minimal-diff).
- `CONVERTED` — two rewrites only: `.bit_length()` becomes `$clog2`, as `($X - 1).bit_length()` → `$clog2($X)` and general `e.bit_length()` → `$clog2(e + 1)`; and a C-style integer literal is re-spelled in SV based form, `0x`/`0o` → `'h` and `0b` → `'b` (`_cStyleIntRespell`).
- `NEEDS_MANUAL` — left untouched with a reason: floor division, real literals, `**`, non-`bit_length` calls, booleans, and any row carrying `valueType: real`.

**Floor division fails loud.** Python `//` floors toward -inf while SV `/` truncates toward zero, so a blind `//` → `/` rewrite would silently corrupt the ceiling idiom `-(-a // b)`. `convertExpr` walks the whole tree for `ast.FloorDiv` and routes every floor-division expression to `NEEDS_MANUAL` so a human rewrites it, a positive ceiling becoming `(a + b - 1) / b`. This is deliberate, not a gap.
...
**Where invoked.** This is Phase A of the unified migration (`migrateYaml.py`). The standalone `migrateEvalExpr.py` CLI from the original plan was superseded and never built.

### 4.4 Schema changes (`schema.py` / `config/schema.yaml`)

- **`flat` attribute** (`schema.py`): a `flat` section gets a `flatData[section][qualifiedKey]` index maintained during parse, distinct from the scoped store. `constants` is now `flat` (`schema.yaml`), which underpins the eval pipeline's qualified lookup.
- **`evalCanonical` field on `constants`** (`schema.yaml`): stores `unparse(node)` for eval rows, empty otherwise. Generator-owned, not user-authored.
- **`value: eval`** (`schema.yaml`) triggers expression parsing. **`maxValue: optional(0)`** and **`isParameterizable: optional(false)`** form the parameterizable-constant contract: an `ipParameters` constant must carry a positive `maxValue`; a derived eval constant must not carry a hand-written `maxValue`.
- **Foreign-key invariants** enforced at schema load (`schema.py`): plain-FK targets must be `flat` and name the storage key; combo-FK source/target combos must match component-wise; every `flat` section must have a populated qualified storage key. These make lookup total functions.
...
`ValueResolver` is a parse-time helper bound to a file context (`valueResolver.py`), created per-file by `processYaml.py` and nulled at the end of each parse phase.

- **Constants and enums.** `lookupNamedRow(key, label)` dereferences an already-qualified key in `flatData['constants']`, falling back to `qualEnums`. `lookupVisibleRow` resolves a bare name by walking the include chain (`global` searches all contexts).
- **Qualified overrides / flat lookup.** `qualifyKey(ref, context, fatal)` qualifies foreign-key columns and eval symbols: numeric values return `''` (literals carry no symbol); bare names resolve to `name/context`; unknowns hard-fail or return `None`.
- **Active versus max resolution.** `value(ref)` → `_resolveActiveValue` honors per-instance variant overrides. `maxValue(key)` → `_resolveMaxValue` returns a parameterizable referent's worst-case bound. Width and struct helpers each choose active or max via a `use_max` flag.

### 4.6 Migration tooling (`yamlFormat: 2`)

`make migrate` wraps `migrateYaml.py --write` (`include/make/a2c-common.mk`). `migrateYaml.py` never opens the database — it reads and rewrites YAML as text only. The three ordered phases (includes, eval Python→SV, addressControl→per-block) and the conditional stamp (Phase C) are described from the user's perspective in §3.3. The hard-stop gate (`_gateYamlFormat` in `processYaml.py`) prints distinct messages for an un-migrated project versus a version mismatch, both exiting non-zero.
...
- `getBlockData(qualBlock, ...)` (`processYaml.py`) — the per-block view; a pipeline of `getBD*` sub-builders, each attaching one facet (instances, registers/memories, address decode, connections, ports, includes, config info, parameterized declarations, address-block view).
- `getContextData(contexts, dataTypeMapping)` (`processYaml.py`) — the per-context view consumed by the constants/types/enums and Config-header emitters.

Every view function named below is **new on this branch** — `git show main:pysrc/processYaml.py` contains none of them.

### 5.2 Config views

**`getBlockConfigView(qualBlock)`** is a memoized per-block bundle of four fields: `isParameterizable`, `hasOwnParams` and `defaultConfig` come from the persisted block row, `variantConfigs` from the descriptors loaded at open. It is an internal helper; templates consume the equivalent fields surfaced on the block/context views.

**`projectCreate.calcVariantConfigDescriptors()`** is where parameterized types resolve to concrete per-variant values. It runs once at database creation and persists the result as the `VARIANTCONFIGDESCRIPTORS` config blob; `projectOpen` loads that blob into `self.variantConfigDescriptors` and the view layer only selects from it.
- The variant set derives from the block's `parameters:` binding rows, so every declared variant gets a descriptor whether or not an instance binds it. A parameterizable block with no binding rows gets an empty list.
- The Config struct's fields are the parameterizable constants of the block's canonical config context (`configContext` on the block row), plus a synthetic field for each block param with no constant of that name.
- **Eval re-resolution under variant overrides is deferred**: eval-derived constants keep their default-resolved value in the descriptor; the symbolic emission (§6.1.2) recomputes them per Config struct.
- **Every declared variant is canonical** and emits its own struct. Two variants whose resolved values are identical still produce two distinct C++ types.

**`selectVariantDescriptor(descriptors, variant, consumerProject, blockProject)`** is the module-level picker both steps share. It returns the one descriptor a consumer binds for a variant, preferring the consumer's own project's declaration, then the same-project one, then the block's declaring project, and `None` when no reachable project declares that variant.

**Every path that emits a parameter VALUE selects a descriptor.** The in-memory `data['parameters'][block]['variants'][variant]['params']` nesting is keyed by bare param name and drops the declaring project, so two projects binding one label collapse onto one slot and the last parsed wins. Reading a value from it puts the SystemC Config and the Verilated SV top of one build at different numbers. `getStandaloneVariants`, `_resolveSvInstanceParams` and the SV wrapper trampoline all go through the descriptor instead. Two details the descriptor carries for them:
- `valueSymbols` is the constant NAME per symbol-bound param, beside the resolved `values`. A sub-block instantiation sits inside the parent module, so it spells the name and a parent parameter of that name flows its value down; a standalone Verilator top has no such scope and uses `values`.
- `BLOCKPARAMDEFAULTS`, persisted beside the descriptors, is each block's declared parameter defaults. When `selectVariantDescriptor` returns `None` the block takes its default Config, and this is what makes the emitted parameters the same numbers.

**`getContextConfigView(contexts)`** is a single-pass context-level view returning `variantConfigs` (aggregated per-block descriptors) and `blockParamSynthetic` (the member-type contract for block params with no backing parameterizable constant). The `context`-prefixed result names (`contextVariantConfigs`, `contextBlockParamSynthetic`) avoid shadowing during SV generation's view merge.

**`_resolveInstanceConfigFields`** resolves per-instance Config typing for children, returning `configName`, `configArg` (the `<X>` template suffix), and flags. Its head comment records the limitation: a Config-strict link from a multi-variant parent to a parameterized child is unsupported, so use a single-variant child, a Config-agnostic interface, or a thunker bind.

### 5.3 Variant resolution and block-param identity

A *variant* is a named (or anonymous) binding of a parameterizable block's params to concrete values. Resolution: instances declare `variant:`; `parameters:` rows bind `(variant, param) → value`; `calcVariantConfigDescriptors` turns those rows into per-variant descriptors at database creation, and the view layer reads them via `getQualBlockVariants` and `selectVariantDescriptor`.
...
- **`getBDAddressDecode(ret)`** determines router-ness purely from the block's own `addressBlock:` field — `isApbRouter` is `True` only when the block authors `addressBlock:`. It surfaces `addressGroupData`, `addressGroup`, `containerBlock`, and `instanceWithRegApb`.
- **`getBDAddressBlockView(ret)`** is deliberately tiny: for a router it copies `blockRow['addressBlock']`; non-routers get nothing.
- **`getBDAddressBus(ret)`** populates `registerBusInterface`, `registerBusPort`, and `registerBusStructs` for any router or decoder.
- **`_registerBusInterfacePort(ret)`** is the unified resolver and embodies the central principle: `registerPorts:` is a **parse-time construct that projectOpen never reads**. Three resolved-design paths: router (port from `addressBlock['upstreamPort']`), synthesized handler (reads the leaf-to-handler `connectionMap`), and routed leaf (reads the same map, filtered to `addressBus: true`).
...
`buildThunkerView` (a closure inside `getBDCrossInterfaceBinds`) builds a language-neutral description of the per-protocol *thunker* that bridges a **cross-interface bind** — a connection whose parent interface differs from the child block's bottom-up declared port interface (including different per-variant Config types on each side).

- It is driven by `interface_defs`: it reads the protocol's `sc_channel.type` and the `parameters` whose `datatype == 'struct'`. **A protocol with no struct parameters produces no thunker**.
- It emits `payloads` for both parent and child sides, ordered by side then declared parameter order.

It is produced inside `getBDCrossInterfaceBinds(ret)`, which walks connections, connection ports, and connection maps. `test_thunker_view.py` locks payload order and channel type per protocol (`rdy_vld`, `req_ack`, `push_ack`, `apb`) and confirms `notify_ack` (no struct params) produces no cross-interface ends.
...
The cross-interface compatibility gate is `validatePorts` in `projectCreate` — it writes nothing back. It requires same `interfaceType`, same `structures` paired by `structureType`, and exact per-field `_bitWidth`/offset under the bound variant. Synthesized binds (`_context == '_global'`) are exempt.

The load-bearing branch (resolved 2026-06-12): `registerPorts:` is part of the declared-port surface, so the check consults the **union `ports: ∪ registerPorts:`**. `test_register_ports_independent_of_ports.py` locks the independence: a leaf may declare a partial `ports:` map plus a `registerPorts:`, and the partial-`ports:` completeness check skips the register-bus port. `test_boundary_signals.py` confirms the view supplies symbolic widths for parameterizable payloads and fixed widths for the register bus, so the HDL/Verilated boundary matches the resolved struct width.
...
The `force_link` mechanism is fully retired: no template declares, emits, or calls one, and `baseClassDecl.py` records the absence outright. Two registration shapes coexist:
- **Non-templated blocks** self-register via a namespace-scope static marked `A2C_REGISTRATION_RETAIN` (defined as `[[gnu::used, gnu::retain]]` in `common/systemc/instanceFactory.h`). The retain attribute survives `--gc-sections`; archive-packaged builds must link with `--whole-archive` (build-system contract).
...
| --- | --- | --- |
| `package.py` | one `<context>_package` per context | `isp_shared/rtl/isp_types_package.sv`, `rtl/debayer_package.sv` |
...
| `moduleRegs.py` | the auto-synthesized register-handler module | `rtl/debayer_regs.sv`, `ip/rtl/ipRegs.sv` |
| `apbDecodeModule.py` | the address-decode router module | `top/rtl/apbDecode.sv` |
...
`moduleInterfacesInstances.py` emits a `#( ... )` parameter list, one `parameter <NAME>` per row — **default-less** (no `= <pkg>::<NAME>` fallback). Module-local declarations follow the port list, produced by the shared helper `package.parameterizedDeclLines()`, ordered by `orderIndex` (eval-derived localparams first, then types/sub-structs before structs that use them).

The **symbol-spelling rule** (`package.parameterizedDeclLines`): a symbol that is one of the block's own params stays symbolic; a symbol already emitted as an earlier localparam is spelled by that localparam; any other symbol is spelled from its persisted value as a literal.

**Instance parameter forwarding** (`moduleInterfacesInstances.py`): if a child's param matches one of the parent block's own params, it forwards the parent symbol; otherwise it emits the child's variant-bound literal.

#### 6.2.3 Parameterized types/structs/constants (eval → SV localparam)

Eval-derived parameterizable constants emit as module-local `localparam`s whose RHS is translated from the persisted `evalCanonical` string via `package.emitSvCanonical` → `emissionUtils.emitExpr(..., SV)`. Operators pass through one-to-one, `$clog2` stays `$clog2`, and `Num` re-spells in its authored unsized SV base. Result (`examples/ip_test/ip/rtl/ip.sv`):
...
The package render filters these out (`package.py::render`): every loop body `continue`s on `isParameterizable`.

#### 6.2.4 Parameterized register decode

`moduleRegs.py` makes the auto-synthesized register handler variant-correct (C3.R). The handler inherits its parent block's params, emits them as module parameters, and emits the parent's `parameterizedDecls` module-local. The parent forwards its params on the handler instance (`rtl/debayer.sv`).

**Per-variant word-count design.** A register is parameterizable iff its storage struct is. Storage is the variant-width module-local struct; the **APB address footprint stays at the worst-case word count** (address constants are byte-identical across variants). For each potential word, the data flop and 32-bit read view are wrapped in a `generate ... if (<W> > 32*gi) ...` ladder with `<W> = $bits(<struct>)`. Absent words are elaborated away and read `'0`. Verified in `examples/ip_test/ip/rtl/ipRegs.sv`.
...
Both SV packages survive, in different places. `rtl/debayer_package.sv` stays at the debayer root. The shared-types package moved out of `rtl/` into the `isp_shared` submodule as `isp_shared/rtl/isp_types_package.sv`, so a superproject `git diff main...HEAD -- rtl/isp_types_package.sv` reads as a whole-file deletion and tells you nothing. What actually changed is that the **parameterizable contents were relocated into module scope**:

- Comparing `main:rtl/isp_types_package.sv` against today's `isp_shared/rtl/isp_types_package.sv` shows the package losing `BITS_PER_PIXEL_COLOR`, `MAX_PIXEL_VALUE`, `PIXELS_PER_CLOCK`, `HORIZONTAL_SIZE`, `VERTICAL_SIZE`, `VERTICAL_SIZE_LOG2`, `pixel_t`, and the structs `rgb_pixel_t`, `bayer_pixels_per_clock_t`, `rgb_pixels_per_clock_t`, `video_bayer_t`, `video_rgb_t`. It retains only non-parameterizable items (`eol_t`/`sof_t`/`eof_t`, `bayer_pattern_t`, `color_t`, `video_frame_t`).
- `git diff main...HEAD -- rtl/debayer.sv` shows the receiving side: `debayer` gains a `#(parameter BITS_PER_PIXEL_COLOR, PIXELS_PER_CLOCK, HORIZONTAL_SIZE, VERTICAL_SIZE)` header, the relocated localparams/typedefs/structs reappear module-local and symbolic (`rtl/debayer.sv`), and `u_preprocess`/`u_interpolate` get literal bindings (they carry explicit `variant: default`) while `u_debayer_regs` gets symbolic forwarding (the reg-handler inherits parent params).
...
1. **Plans are status records, not always current code.** `plan-eval-symbolic-emission.md` lists E1.5 (`evalPyToSv.py`) as "DEFERRED" in one table, but the code is landed and exercised. The code is authoritative. `plan-116-status-report.md` cautions that "Complete" refers to committed implementation, not to the (untracked) plan files themselves.
...
7. **Stale narrative comments.** `classDecl.py` holds the generator's last force-link reference, a code comment claiming non-templated blocks "additionally carry an active force-link function in `<block>Base.h`". Both halves are stale. The mechanism was retired under Option δ, and `baseClassDecl.py` now says outright that no force-link declaration is emitted; the base declaration also moved from `<block>Base.h` to a `<block>Base.cppm` module unit. Nothing generated is affected, because the comment is descriptive only. The phrase does survive in the registration comments `testbench.py` emits into `<block>Testbench.cppm` and `<block>Config.cpp`, but there it correctly describes the absence: main() reaches registration "with no force-link reference". `structures.py` carries a stale `# TODO: handle variable size parameters`; the wrapper path already handles parameterizable widths.
