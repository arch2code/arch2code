...
- **Step 17 follow-ups LANDED 2026-09-11, committed as a9267f7, gates VERIFIED BY EXECUTION on the
  final tree.** All seven ruled and applied (§6, step 17, "Follow-ups ruled" and after): three `make db`
  ...
  `make agents-setup` run in the product, so every deployed skill matches its source. Committed
  2026-09-11 as a9267f7.
- **Step 15 follow-up (a) LANDED 2026-09-11.** The empty `<context>VariantConfig.h` is retired:
  fileMap entry, scaffold, renderer and validity gate removed; `make migrate` deletes a surviving
  copy in either layout and reports a hand `#include` of it. See §6, step 15. Found on the way and
  RULED the same day: a block's Config module follows `hasOwnParams` alone, the superset rule (§6,
  step 15, "Found on the way, RULED"). `make ip-test -j` with the re-scaffolded
  `ip_ipRegsVariantConfig.cppm` present: exit 0, 18 "No error", the module compiled. At the user's
  request `make newmodule -j` was then run in every project tree (53 in `builder/base` including
  composed children bottom-up, 2 in `builder/pro`, `isp_shared` and the product), one at a time,
  with before/after `git status` snapshots: the only change anywhere is three new registrar Config
  modules in `examples/xprojParam/cpLayout` (`xpCpLayout_xpCpLeaf`, `xpCpSrc`, `xpCpWrap`
  `VariantConfig.cppm`), filled by `make gen`; nothing was deleted. The four non-zero exits are the
  four deliberate rejection fixtures (`cpLayoutBad`, `inhLayoutBad`, `varUniqBad`, `mtxBare`), each
...
- **Follow-up (e), TESTED 2026-09-10.** The endpoint diagnostic "The block declares
  ...
  it. Skill drift found on the way, since CLOSED: `design-parameterizable-blocks` rule 1 said root
  parameters live in the block's own YAML file; the wording was corrected in d155d09.
