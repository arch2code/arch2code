...
### Step 4 — B3: `foreignConfigModules` in the testbench External. PARALLEL, depends on nothing

- **Changes.** `ext_module_export` (`templates/systemc/testbench.py:228-249`) emits the same imports the block-module path already emits from `data['foreignConfigModules']`.
- **Proves it.** ~~Remove `cstBind`'s wrapper so the cells sit directly under `xpCstBindTop`, and require `make xproj-const` to build and run.~~ **This proof is now vacuous and must be replaced.** The `xprojParam` testbench Externals were retargeted at their `_tb` container (`--block=<X>_tb --excludeInst=u_<X>`), so an External now holds the DUT's siblings rather than the DUT's own children. `cstBind`'s External therefore names no foreign Config whether or not B3 is fixed, and removing the wrapper would pass either way. A replacement gate must put a block whose Config is foreign into the External's OWN instance set — that is, a sibling of the DUT inside the `_tb` container — rather than under the DUT.
- **Blast radius.** Additive imports on one generated file kind. Expected small; to be measured by hashing.
...
Steps 2, 4 and 5 depend on nothing in this plan and may be taken in any order alongside step 1. Steps 3 and 6 wait on step 1. Q3's rename waits on step 3. Step 7 depends on step 1 only (its measurements were taken against the post-step-1 generator) and **not** on Case 1 (§2, D2); Case 1 in turn waits on step 3. Nothing here depends on [`plan-interface-compatibility.md`](./plan-interface-compatibility.md), whose steps 1 through 8 have already landed; step 1 of this plan **removes** that plan's sole remaining failing unit cell.
