...
- **Follow-up, OPEN.** `pysrc/newModule.py` ~90-96 scaffolds a bare `<block>_hdl_sv_wrapper.sv` for a `hasVl` block with no standalone variant. A design authored container-sourced from the start gets that file, `render_sv` renders it as a non-parameterizable module over a default-less DUT, and the manifest now leaves it out of the top set silently. `newModule` should not scaffold the bare file for a params-declaring block, or the manifest should report it.
...
- **Follow-up.** The diagnostic for an inheriting end says "at its container's configuration" without naming the container block and variant; with a multi-variant container the reader cannot tell which site supplied the width. The containerParam path has the same gap since step 7b. Fix: junctionBindings yields the governing containerSite with each pair, checkInterfacePair passes it to _junctionSideIdentity, and a multi-variant rejecting fixture proves it. Three call sites unpack the pair: two in validatePorts, one in config/postParseRegisterPorts.py.
