...
class genBase : public virtual blockPortBase
{
public:
    virtual ~genBase() = default;
    // src ports
    // datIf->u_leaf0: data interface
    push_ack_out< dat_st > dOut;

    // dst ports
    // u_leaf1->datIf: data interface
    push_ack_in< dat_st > dIn;


    genBase(std::string name, const char * variant) :
        dOut("dOut")
        ,dIn("dIn")
    {};
...
class genInverted : public virtual blockPortBase
{
public:
    // src ports
    // datIf->u_leaf0: data interface
...
public:
    ...
    genChannels(std::string name, std::string srcName) :
    dOut(("dOut"+name).c_str(), srcName)
    ,dIn(("dIn"+name).c_str(), srcName)
    {};
    void bind( genBase *a, genInverted *b)
    {
        a->dOut( dOut );
        b->dOut( dOut );
        a->dIn( dIn );
