...
// GENERATED_CODE_PARAM --block=vliTop
...
class vliTopConfig : public testBenchConfigBase
{
public:
    virtual ~vliTopConfig() override = default; // Explicit Virtual Destructor
...
protected:
    ...
    std::shared_ptr<blockBase> createTbTop(void) { return instanceFactory::createInstance("", "tb", "vliTopTestbench", "", "vlInh"); }
public:
// GENERATED_CODE_END
...
// === Testbench config registration (vliTopConfig) ===
...
void register_vliTopConfig() {
    testBenchConfigFactory::registerTestBenchConfig("vliTop", [](std::string) -> std::shared_ptr<testBenchConfigBase> { return static_cast<std::shared_ptr<testBenchConfigBase>> (std::make_shared<vliTopConfig>());}, is_default_testbench_v<vliTopConfig>);
}

namespace {
[[maybe_unused]] A2C_REGISTRATION_RETAIN int _vliTopConfig_registered = (register_vliTopConfig(), 0);
} // namespace
