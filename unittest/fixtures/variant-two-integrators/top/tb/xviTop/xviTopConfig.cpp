...
class xviTopConfig : public testBenchConfigBase
{
public:
    virtual ~xviTopConfig() override = default; // Explicit Virtual Destructor
...
protected:
    ...
    std::shared_ptr<blockBase> createTbTop(void) { return instanceFactory::createInstance("", "tb", "xviTopTestbench", "", "xviTop"); }
public:
// GENERATED_CODE_END
...
// === Testbench config registration (xviTopConfig) ===
...
void register_xviTopConfig() {
    testBenchConfigFactory::registerTestBenchConfig("xviTop", [](std::string) -> std::shared_ptr<testBenchConfigBase> { return static_cast<std::shared_ptr<testBenchConfigBase>> (std::make_shared<xviTopConfig>());}, is_default_testbench_v<xviTopConfig>);
}

namespace {
[[maybe_unused]] A2C_REGISTRATION_RETAIN int _xviTopConfig_registered = (register_xviTopConfig(), 0);
} // namespace
