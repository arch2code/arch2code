...
// GENERATED_CODE_PARAM --block=xviMidTop
...
class xviMidTopConfig : public testBenchConfigBase
{
public:
    virtual ~xviMidTopConfig() override = default; // Explicit Virtual Destructor
...
protected:
    ...
    std::shared_ptr<blockBase> createTbTop(void) { return instanceFactory::createInstance("", "tb", "xviMidTopTestbench", "", "xviMid"); }
public:
// GENERATED_CODE_END
...
// === Testbench config registration (xviMidTopConfig) ===
...
void register_xviMidTopConfig() {
    testBenchConfigFactory::registerTestBenchConfig("xviMidTop", [](std::string) -> std::shared_ptr<testBenchConfigBase> { return static_cast<std::shared_ptr<testBenchConfigBase>> (std::make_shared<xviMidTopConfig>());}, is_default_testbench_v<xviMidTopConfig>);
}

namespace {
[[maybe_unused]] A2C_REGISTRATION_RETAIN int _xviMidTopConfig_registered = (register_xviMidTopConfig(), 0);
} // namespace
