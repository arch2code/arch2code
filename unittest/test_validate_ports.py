...
def test_nested_example_passes():
    ...
    # (include/make/shared.mk A2C_PRJ_YAML); the build runs from the example root.
    nested_dir = os.path.join(base_dir, 'examples', 'nested')
    project_path = os.path.join(nested_dir, 'prj', 'yaml', 'nestedProject.yaml')
    db_path = tempfile.mktemp(suffix='.db', dir=test_dir)
    try:
